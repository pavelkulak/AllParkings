...
export const getPendingParkings = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('http://localhost:3000/api/parking-lots/pending');
...
export const updateParkingStatus = createAsyncThunk(
  ...
  async ({ parkingId, status }: { parkingId: number; status: ParkingStatus }, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://localhost:3000/api/parking-lots/${parkingId}/status`, { status });
      return response.data;
    } catch (error) {
